...
function start() {
    ...
    dc.onmessage = function (messageEvent) {
        statusField.innerText = "Listening... plz say something";
        ...
        try {
            ...
            console.log("Result1", voskResult)
        } catch (error) {
            console.error(`ERROR: ${error.message}`);
            return;
        }
        if ((voskResult.text?.length || 0) > 0) {
            //console.log("Result2", voskResult.text)
            performRecvText(voskResult.text);
        } else if ((voskResult.partial?.length || 0) > 0) {
            //console.log("Result Partial", voskResult.partial)
