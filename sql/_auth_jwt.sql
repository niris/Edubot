...
begin
  ...
  if _role is null then raise invalid_password using message = 'invalid password'; end if;
  select sign((r), current_setting('app.settings.jwt_secret')) as token from (
    select
      _role as role,
      login.id as id,
...
$$ language plpgsql STABLE; -- IMMUTABLE are cached, STABLE can be GET
GRANT EXECUTE ON FUNCTION logout TO "anon", "user";
